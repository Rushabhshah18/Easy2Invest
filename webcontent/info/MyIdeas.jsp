...
<thead>
							<tr>
								...
								<th>ID</th>
								<th class="hidden-xs">Entreprenure Name</th>
								<th>Abstract Idea</th>
						...
						<%
							List<idea> list_permanantidea=(List<idea>)(session.getAttribute("permenant_Idealist"));//ahiya get karine print karavu 6u bus.
																	if(!list_permanantidea.isEmpty() && list_permanantidea!=null){
																	Iterator<idea> its=list_permanantidea.iterator();
																	idea idea=null;
																	while(its.hasNext()){
																		idea=(idea)its.next();
						%>
						<tbody>
							<tr>
								<td><input type="checkbox" name="ideaUser"
									value="<%=idea.getEmail()%>"></td>
								<td><%=idea.getId()%></td>
								<td class="hidden-xs"><%=idea.getEntreprenureName()%></td>
								<td><%=idea.getAbstractIdea()%></td>
								<td class="hidden-xs"><%=idea.getCity()%></td>
								<td><%=idea.getEmail()%></td>
								<td><%=idea.getAmount()%></td>
