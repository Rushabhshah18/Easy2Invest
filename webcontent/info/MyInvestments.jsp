...
<thead>
						...
						<%
							List<proposal> permenant_Proposallist=(List<proposal>)(session.getAttribute("permenant_Proposallist"));//ahiya get karine print karavu 6u bus.
																	if(!permenant_Proposallist.isEmpty() && permenant_Proposallist!=null){
																	Iterator<proposal> its=permenant_Proposallist.iterator();
																	proposal proposal=null;
																	while(its.hasNext()){
																		proposal=(proposal)its.next();
						%>
						<tbody>
							
												<tr>
													<td><%=proposal.getId()%></td>
													<td class="hidden-xs"><%=proposal.getSender()%></td>
													<td><%=proposal.getReceiver()%></td>
													<td class="hidden-xs"><%=proposal.getAmount()%></td>
													<td><%=proposal.getProposalIdea()%></td>
													<td><%=proposal.getCity()%></td>
													<td class="hidden-xs"><%=proposal.getEmail()%></td>
													<td><a
														href="<%=request.getContextPath()%>/proposal?actionCode=doUpdateIdea&&id=<%=proposal.getId()%>"
													...
													%>
													<td><a
														href="<%=request.getContextPath()%>/proposal?actionCode=doDeleteIdea&&id=<%=proposal.getId()%>"
														class="btn btn-red tooltips" data-placement="top"
