						...
						<div class="toolbar row">
							<div class="col-sm-6 hidden-xs">
								<div class="page-header">
									<h1>Investors <small>Investors Available</small></h1> 
								</div>
							</div>
						...
						<div class="row">
							<div class="col-md-12">
								...
								System.out.println(currentSessionEmail);
									List<investor> list_investor=(List<investor>)(session.getAttribute("list_investor"));
																									if(!list_investor.isEmpty() && list_investor!=null){
																										Iterator it=list_investor.iterator();
																										investor investor=null;
																										
																										while(it.hasNext()){
																											investor=(investor)it.next();
								%>
								<tbody>
									...
									<tr>
										...
										<td class="hidden-xs"><%=investor.getfirstname()%></td>
										<td><%=investor.getLastname()%></td>
										<td class="hidden-xs"><%=investor.getEmail()%></td>
										
										<td class="hidden-xs"><%=investor.getGender()%></td>
										<td><%=investor.getBirthdate()%></td>
										<td class="hidden-xs"><%=investor.getAddress()%></td>
										<td><%=investor.getCreated_date()%></td>
										<%if(differentUser.equals("admin") || differentUser.equals("Admin") ){ %>
										<td><a
											href="<%=request.getContextPath()%>/Invest?actionCode=doUpdateInvestorUser&&id=<%=investor.getId()%>" class="btn btn-light-blue tooltips" data-placement="top" data-original-title="Edit"><i class="fa fa-edit"></i></a></td>
										<td><a
											href="<%=request.getContextPath()%>/Invest?actionCode=doDeleteInvestor&&id=<%=investor.getId()%>" class="btn btn-red tooltips" data-placement="top" data-original-title="Remove"><i class="fa fa-times fa fa-white"></i></a></td>
										<%-- <td><a id="successSweetAlert"
														class="btn btn-primary pull-right"
														href="<%=request.getContextPath()%>/Invest?actionCode=doSendMail&&email=<%=investor.getEmail()%>&&id=<%=investor.getId()%>&&differentUser=<%=differentUser%>&&currentSessionEmail=<%=currentSessionEmail%>">
															Send Mail </a></td> --%>
															<%} %>
									</tr>
									<%
